...
var _ = Describe("Region Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind Region")
			err := k8sClient.Get(ctx, typeNamespacedName, region)
			if err != nil && errors.IsNotFound(err) {
				resource := &halcyonv1alpha1.Region{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance Region")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &RegionReconciler{
				Client: k8sClient,
